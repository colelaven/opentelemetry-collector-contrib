...
func TestWithFallback(t *testing.T) {
	srv := New(t,
		...
		WithFallback(Response{StatusCode: http.StatusAccepted}),
	)
...
func TestWithRoute_MultiplePaths(t *testing.T) {
	srv := New(t, nil,
		WithRoute("/ingest/logs", []Response{
			{StatusCode: http.StatusBadGateway},
			{StatusCode: http.StatusOK},
		}),
		WithRoute("/ingest/metrics", []Response{
...
func TestWithRouteFallback(t *testing.T) {
	srv := New(t, nil,
		WithRoute("/api/data",
			...
			WithRouteFallback(Response{StatusCode: http.StatusCreated}),
		),
	)
